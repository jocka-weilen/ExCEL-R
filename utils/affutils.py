...
def refine_cams_with_bkg_weclip(cam_refined_list, inputs_denorm, cls_lst,par, size,device="cuda"):

    w, h = size
    cams = generate_cam_label(cam_refined_list, h, w).to(device)
    bg_score = torch.pow(1 - torch.max(cams, dim=0, keepdims=True)[0], 1.).to(device)
    cams = torch.cat([bg_score, cams], dim=0).to(device)

    valid_key = np.pad(cls_lst.numpy()  + 1, (1, 0), mode='constant')
    valid_key = torch.from_numpy(valid_key).to(device)
...
def refine_cams_with_aff(attr_map, attn_weights,cls_label, size,caa_thre =0.79, attn_layers=6, seg_attn=None,device='cuda'):
