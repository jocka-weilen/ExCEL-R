...
当前实现版本：**v11.5-NT（无教师模型）**。

CoSeR-CLIP 是在 ExCEL 弱监督语义分割代码库上迭代得到的、面向类别共现与语义混淆的 CLIP 有符号证据路由方法。当前实现对应方法稿 `CoSeR_CLIP_Method_v11_5_NT_CN_AAAI27.tex`，方法名在新增模型、训练、评测、日志和 checkpoint 中统一为 **CoSeR-CLIP**。
...
| 混淆感知区域所有权 | `model/coser_core.py` | 可学习区域查询、外观—空间区域图、Top-K 混淆直接归一化、正负深层锚点对称停止梯度 |
...
| 完整训练目标 | `model/losses.py` | 分类、缺失困难负类路由、停止梯度在线掩码、结构排序、空间分配重叠与查询使用损失 |
| 视觉分割分支 | `model/model_coser_clip.py` | 冻结 CLIP 多层视觉特征融合；分割头不接收 CAM |

训练前 `warmup_iters` 次迭代仅启用正向证据；之后才启用 Top-K 区域竞争、图像缺失困难负类路由和负向抑制。区域所有权路径中的正、负深层锚点均停止梯度，深层与中层目标支持使用平滑 OR 融合。该版本不使用教师模型、弱—强双视图、周期标签缓存或新增网络分支。
