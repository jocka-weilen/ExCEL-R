...
# parser.add_argument("--model", default="../clip/pretrained/ViT-B-16.pt", type=str, help="custom clip")
...
parser.add_argument("--log_iters", default=2, type=int, help=" logging iters")
# parser.add_argument("--eval_iters", default=2000, type=int, help="validation iters")
parser.add_argument("--eval_iters", default=2, type=int, help="validation iters")
...
def train(args=None):
    ...
    use_ddp = (args.local_rank != -1) and use_cuda
    ...
    if use_cuda:
        if use_ddp:
            ...
            logging.info("DDP GPU mode | Total gpus: %d, samples per gpu: %d", world_size, args.spg)
        else:
            # 单卡 GPU
            torch.cuda.set_device(0)
            world_size = 1
            logging.info("Single GPU mode | Total gpus: %d, samples per gpu: %d", world_size, args.spg)
    else:
        # CPU
        world_size = 1
        logging.info("CPU mode | Total devices: %d, samples per device: %d", world_size, args.spg)
    ...
    if device != "cpu":
        ...
        par = PAR(num_iter=20, dilations=[1, 2, 4, 8, 12, 24]).to("cpu")
    ### build dataloader 
    ...
    if use_ddp:
        # 多卡 DDP 下使用 DistributedSampler
        train_sampler = DistributedSampler(train_dataset, shuffle=True)
        # 用随机整数初始化 shuffle seed（兼容原来的 np.random.randint 逻辑）
        train_sampler.set_epoch(np.random.randint(args.max_iters))
        shuffle = False  # DataLoader 不需要再 shuffle
    else:
        train_sampler = None
        shuffle = True  # 单卡 / CPU 可直接 shuffle
    ...
    for n_iter in range(args.max_iters):
        ...
        for i, attr_map in enumerate(attr_maps_raw):
            ...
            refined_attr_maps, cls_lst = refine_cams_with_aff(attr_map, attn_weight, cls_label, size=inputs.shape[2:], seg_attn=seg_attn, caa_thre=0.79,device="cuda" if args.device != "cpu" else "cpu")
            attr_aff_labels,_ = refine_cams_with_bkg_weclip(refined_attr_maps, inputs_denorm[i], cls_lst, par, size=inputs.shape[2:],device="cuda" if args.device != "cpu" else "cpu")
        ...
        _, diver_pseudo_labels = lam_to_label(attr_maps_raw.permute(0,2,1).reshape(4,20,20,20),
                                              cls_labels.to("cuda" if args.device != "cpu" else "cpu"),
                                              img_box=None,
                                              ignore_mid=False,
        ...
        if (n_iter + 1) % args.eval_iters == 0 and (n_iter + 1) >= 1:
            ...
            tab_results = build_validation(model=model, par=par, val_loader=val_loader,device="cuda" if args.device != "cpu" else "cpu")
            if args.local_rank == 0:
                logging.info("\n"+tab_results)
