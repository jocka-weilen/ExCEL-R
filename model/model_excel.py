...
class ExCEL_model(nn.Module):
    def __init__(self,  clip_model=None, embedding_dim=256, in_channels=512, dataset_name='pascal_voc', \
        ...
        self.text_attr, self.attr_flag = attr_aggregate(self.integral_text_features, dataset_name, num_classes-1, num_atrr_clusters, json_file,device=device)
    ...
    def forward(self, img, ex_feats=None):
        ...
        b, c, h, w = img.shape
        self.encoder.eval()
        image_features, attn_weights, all_feats = clip.generate_clip_fts(img, self.encoder, return_weights=True,device=self.device)
